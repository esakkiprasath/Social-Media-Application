...
def index1():
    ...
    if request.method=='POST':
        ...
        pwd=request.form['pass']
        cursor = mydb.cursor()
        cursor.execute('SELECT * FROM ds_register1 WHERE uname = %s AND pass = %s', (uname, pwd))
        account = cursor.fetchone()
        if account:
            session['username'] = uname
    ...
    return render_template('insta/index.html',msg=msg)
...
def change_pw():
    ...
    cursor = mydb.cursor()
    cursor.execute("SELECT * FROM ds_register where uname=%s",(uname,))
    ...
    if request.method=='POST':
        ...
        cursor.execute('SELECT count(*) FROM ds_register WHERE uname = %s AND pass = %s', (uname, oldpass))
        account = cursor.fetchone()[0]
        if account>0:
            cursor.execute("update ds_register set pass=%s where uname=%s",(newpass,uname))
            mydb.commit()
            msg="success"
        else:
            msg = 'fail'
...
def register1():
    msg=""
    mycursor = mydb.cursor()
    mycursor.execute("SELECT max(id)+1 FROM ds_register1")
    maxid = mycursor.fetchone()[0]
    if maxid is None:
        maxid=1
    if request.method=='POST':
        ...
        mycursor.execute("SELECT count(*) FROM ds_register1 where uname=%s or aadhar=%s",(uname,aadhar))
        cn = mycursor.fetchone()[0]
        if cn==0:
            now = datetime.now()
            rdate=now.strftime("%d-%m-%Y")
        
            sql = "INSERT INTO ds_register1(id,name,gender,dob,mobile,email,location,profession,aadhar,uname,pass,rdate) VALUES (%s,%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
    ...
    return render_template('insta/register.html',msg=msg)
...
def userhome1():
    ...
    if 'username' in session:
        uname = session['username']
    mycursor = mydb.cursor()
    mycursor.execute("SELECT * FROM ds_register1 where uname=%s",(uname,))
    data = mycursor.fetchone()

    return render_template('insta/userhome.html',msg=msg,data=data)

@app.route('/user_post2', methods=['GET', 'POST'])
def user_post2():
    msg=""
    st=0
    uname=""
    ...
    if 'username' in session:
        uname = session['username']
    cursor = mydb.cursor()
    cursor.execute('SELECT * FROM ds_register1 WHERE uname = %s', (uname, ))
    data = cursor.fetchone()

    pcursor = mydb.cursor()
    pcursor.execute('SELECT * FROM ds_post1 u,ds_register1 r where u.uname=r.uname && u.status<=1 order by u.id desc')
    pdata = pcursor.fetchall()

    pcursor1 = mydb.cursor()
    pcursor1.execute('SELECT count(*) FROM ds_post1 WHERE uname = %s and status=1', (uname, ))
    ...
    if request.method == 'POST':
        ...
        mycursor.execute("SELECT max(id)+1 FROM ds_post1")
        maxid = mycursor.fetchone()[0]
        if maxid is None:
            maxid=1
        ...
        if file.filename == '':
            ...
            #return redirect(request.url)
        if file:
            fname = "S"+str(maxid)+file.filename
        ...
        sql = "INSERT INTO ds_post1 (id,uname,text_post,photo,rdate,status,user,pre_id) VALUES(%s,%s,%s,%s,%s,%s,%s,%s)"
        val = (maxid,uname,post,file_name,rdate,sta,user,pre_id)
        mycursor.execute(sql,val)
        print(sql,val)
        mydb.commit()
        if sta=='1':
            ...
            pre_vid=sp2[0]
            email1=sp2[4]
            mess1="Similar Image upload in other social app, Post by "+uname
            print(mess1)
            print(email1)

            bdata1="ID:"+str(pre_vid)+", Username:"+pre_user+", Status:Attack Found, Similar Post by Other soical app, "+uname+", Post ID:"+str(maxid)+", (Previous ID:"+str(pre_id)+"), Date:"+dtime
            bdata11="ID:"+str(maxid)+", Username:"+uname+", Status:image already exist, PID:"+str(maxid)+", Date:"+dtime
            msg1="attack"
    ...
    if act=="del":
        ...
        mycursor.execute("delete from ds_post1 where id=%s",(did,))
        mydb.commit()
        return redirect(url_for('user_post2'))
    
    return render_template('insta/user_post.html',msg=msg,uname=uname,data=data,act=act,pdata=pdata,mess1=mess1,email1=email1,bdata2=bdata2)
...
def blurImage(img,pid):
    ...
    for (x, y, w, h) in face_data:
        ...
        #roi = cv2.GaussianBlur(roi, (28, 28), 50)
        roi = cv2.medianBlur(roi,(35))
        # impose this blurred image on original image to get final image
        image[y:y+roi.shape[0], x:x+roi.shape[1]] = roi
        gg="b"+pid+"_"+str(j)+".jpg"
        cv2.imwrite("static/group/"+gg, roi)
        
        im1 = Image.open("static/comments/F"+img)
        im2 = Image.open("static/group/"+gg)
        back_im = im1.copy()
        back_im.paste(im2, (x, y))
        back_im.save('static/comments/F'+img, quality=95)
...
def userhome():
    ...
    if request.method == 'POST':
        ...
        if file:
            ...
            if pmode=="2":
                ...
                #########
                '''image = cv2.imread("static/comments/"+file_name)
        ...
        if ss=="ok":
            ...
            pre_vid=sp2[0]
            email1=sp2[4]
            mess1="Similar Image upload, Post by "+uname
        ...
        mess="Dear "+name+", You have sent new post, Post ID:"+str(maxid)+", Date / Time:"+dtime
        ###
        if pmode=="1":
            msg="suc"
            pid=str(maxid)
    ...
    if act=="confirm":
        ...
        cbq411 = mycursor.fetchone()
        email=cbq411[4]

        mess="Friend Request accepted by "+uname
    ...
    if act=="reject":
        ...
        mess="Friend Request Rejected by "+uname
    ...
    if act=="send":
        ...
        mess="Friend Request from "+uname
    ...
    if act=="acc":
        ...
        mycursor.execute('SELECT * FROM ds_register where uname=%s',(cuser,))
        cbq411 = mycursor.fetchone()
        email=cbq411[4]

        mess="Post Request accepted by "+uname
    ...
    if act=="dec":
        ...
        mycursor.execute('SELECT * FROM ds_register where uname=%s',(cuser,))
        cbq412 = mycursor.fetchone()
        email=cbq412[4]

        mess="Post Request Declined by "+uname
    ...
    if act=="pblock":
        ...
        mycursor.execute("update ds_post1 set status=3 where id=%s",(aid,))
        mydb.commit()
        msg="pblk"

    if act=="pacc":
        aid=request.args.get("aid")
        mycursor.execute("update ds_post1 set status=0 where id=%s",(aid,))
        mydb.commit()
        msg="pact"

        
    #####similar post from another social app################
    adata=[]
    s5=""
    
    mycursor.execute('SELECT count(*) FROM ds_post1 where user=%s && status=1',(uname,))
    ann = mycursor.fetchone()[0]
    if ann>0:
        s5="1"
        mycursor.execute('SELECT * FROM ds_post1 where user=%s',(uname,))
    ...
    return render_template('userhome.html',msg=msg,data=data,mess=mess,act=act,st=st,pdata=pdata,vid=vid,data2=data2,data3=data3,data4=data4,s1=s1,s2=s2,s3=s3,s4=s4,bc=bc,bdata=bdata,pid=pid,data5=data5,msg1=msg1,bdata2=bdata2,email=email,mess1=mess1,email1=email1,s5=s5,adata=adata)
...
def set_photo():
    ...
    shutil.copy("static/comments/"+fn,"static/comments/F"+fn)
    ...
    if request.method == 'POST':
        ...
        for (x, y, w, h) in face_data:
            ...
            if kk in garr:
                cv2.rectangle(image, (x, y), (x + w, y + h), (0, 255, 0), 2)
                roi = image[y:y+h, x:x+w]
                roi = cv2.GaussianBlur(roi, (23, 23), 5)
                image[y:y+roi.shape[0], x:x+roi.shape[1]] = roi

                gg="b"+pid+"_"+str(k)+".jpg"
                im1 = Image.open("static/comments/F"+fn)
                im2 = Image.open("static/group/"+gg)
                back_im = im1.copy()
                back_im.paste(im2, (x, y))
                back_im.save('static/comments/F'+fn, quality=95)
...
def share():
    ...
    if request.method == 'POST':
        ...
        sql = "INSERT INTO ds_post1 (id,uname,text_post,photo,rdate,status,user,pre_id) VALUES(%s,%s,%s,%s,%s,%s,%s,%s)"
        val = (maxid,shareto,post,file_name,rdate,'0',uname,'0')
        mycursor.execute(sql,val)
        mydb.commit()

        bdata="ID:"+str(vid)+", Username:"+uname+", Status:Shared to "+shareto+", Share by "+uname+", Date:"+dtime
        ##
        msg="success"
        #return redirect(url_for('userhome'))
    ...
    return render_template('share.html',msg=msg,data=data,act=act,bc=bc,bdata=bdata)
...
def user_post():
    ...
    return render_template('user_post.html',msg=msg,data=data,mess=mess,act=act,st=st,pdata=pdata,vid=vid,bc=bc,bdata=bdata)
...
def user_req():
    ...
    return render_template('user_req.html',msg=msg,data=data,mess=mess,act=act,st=st,pdata=pdata,vid=vid,bc=bc,bdata=bdata)
...
def user_block():
    ...
    mycursor.execute("SELECT count(*) FROM ds_request where uname=%s",(uname,))
    cc = mycursor.fetchone()[0]
    if cc>0:
        st="1"
        mycursor.execute("SELECT * FROM ds_request r,ds_post p where r.pid=p.id && r.uname=%s",(uname,))
        data2 = mycursor.fetchall()
        
    
    return render_template('user_block.html',msg=msg,data=data,mess=mess,act=act,uname=uname,key=key,st=st,data2=data2)
